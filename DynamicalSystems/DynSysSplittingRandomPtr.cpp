...
#include "DynSysSplittingRandomPtr.hpp"
...
DynSysSplittingRandomPtr::DynSysSplittingRandomPtr(ThreadRandom *thread) :
    DynSysSplittingPtr(thread),
    cos_kernel_at_spatial_point_(kL, 0.0),
    sin_kernel_at_spatial_point_(kL, 0.0),
    convolutions_(kK, std::vector<Real>(kL, 0.0)),
    normalizations_(kK, 0.0)
{
  for (int i = 0; i < kL; ++i)
  {
...
DynSysSplittingRandomPtr::~DynSysSplittingRandomPtr()
{

}

//void DynSysSplittingRandomPtr::CalculateFlux(Dimension dim)
//{
...
//  for (int j : thread_->GetAngularVelocityLoopIndices())
//  {
//    CalculateConvolution(convolutions_[j], normalizations_[j]);
//  } // j
//  // todo: synchronize convolutions_ and normalizations_
...
//        }
//        break;
//      case Dimension::kOmega: CalculateVelocityAtCellInterface(i, j, velocity, convolutions_[j], normalizations_[j]);
...
void DynSysSplittingRandomPtr::CalculateAngularFlux()
{
  Real density_next_in_current_cell = 0.0, density_prev_in_next_cell = 0.0;
...
void DynSysSplittingRandomPtr::CalculateAngularVelocityFlux()
{
  Real density_next_in_current_cell = 0.0, density_prev_in_next_cell = 0.0;
  int idx_next = 0, i = 0, j = 0;
  Real velocity = 0.0;

  CalculateDensitySlopes(Dimension::kOmega, std::string("density_slope_wrt_angle_window"), density_slope_wrt_angle_);
  ...
  {
    utilities::OneDimIdxToTwoDimIdx(idx, i, j);
    CalculateVelocityAtCellInterface(i, j, velocity, convolutions_[j], normalizations_[j]);
    if (velocity > 0.0)
    {
 ...
 * @param velocities
 */
void DynSysSplittingRandomPtr::CalculateVelocityAtCellInterface(int i,
                                                                int j,
                                                                Real &velocity,
 ...
 * @param potentials
 */
void DynSysSplittingRandomPtr::CalculateVelocityPotentialAtCellCenter(int i,
                                                                      int j,
                                                                      Real &potential,
                                                                      const std::vector<Real> &convolution,
                                                                      Real normalization)
{
  ...
  if (kDiffusionConstant > Real(0))
  {
    ...
    potential += (kDiffusionConstant * std::log(rk_system_state_[idx]));
  }
}

void DynSysSplittingRandomPtr::CalculateConvolution(std::vector<Real> &convolution, Real &normalization)
{
